...
public class CoursesController(IMediator mediator) : ControllerBase
{
    ...
    {
        return Ok(await mediator.Send(new CreateCourseCommand(request.Title, request.Description, 
    ...
    {
        return Ok(await mediator.Send(new GetCoursesQuery(request.Limit, request.Cursor ?? "")));
    }

    [HttpGet("{CourseId}")]
    public async Task<IActionResult> GetCourseById([FromRoute] GetCourseByIdRequest request)
    {
        return Ok(await mediator.Send(new GetCourseByIdQuery(request.CourseId)));
    }
    ...
    {
        return Ok(await mediator.Send(new DeleteCourseCommand(request.CourseId)));
    }

    [HttpPut("{CourseId}")]
    public async Task<IActionResult> UpdateCourse([FromRoute] Guid CourseId, [FromBody] UpdateCourseRequest request)
    {
        return Ok(await mediator.Send(new UpdateCourseCommand(CourseId, request.Title, request.Description,
